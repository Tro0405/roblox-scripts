-- Fly + Noclip + Save Position + Speed Slider
-- AUTO RECONNECT SAU RESPAWN
...
-- ===== TRẠNG THÁI TOÀN CỤC =====
-- Những biến này KHÔNG reset khi chết
local flyEnabled    = false
local noclipEnabled = false
local savedPositions = {}

-- Những biến này luôn lấy từ character hiện tại
...
local function startFlyInternal()
    ...
    flyLoop = RunService.Heartbeat:Connect(function()
        ...
        -- Nếu rootpart đổi (chết/respawn) -> tự reconnect
        if not root or not bodyVel or not bodyVel.Parent then
            return
        end
...
local function onCharacterAdded(newChar)
    ...
    task.wait(0.35)  -- Chờ physics ổn định

    -- TỰ ĐỘNG BẬT LẠI nếu trước đó đang bật
    if flyEnabled then
        startFlyInternal()
    end
...
-- Nếu character đã tồn tại ngay lúc chạy script
...
--             GUI
-- ==============================
-- Xóa GUI cũ nếu chạy lại script
local old = game:GetService("CoreGui"):FindFirstChild("FlyToolProGUI")
if old then old:Destroy() end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name         = "FlyToolProGUI"
...
local function makePill(y, icon, label, color, onToggle)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        sLbl.Text      = state and "DANG CHAY" or "TAT"
...
local function makePositionSection(startY)
    ...
    emptyLbl.Text                 = "Chua co vi tri nao duoc luu"
...
--         BUILD LAYOUT
-- ==============================
section(50, "CHUC NANG")
makePill(66, "✈", "BAY (Fly)", Color3.fromRGB(70, 120, 255), function(on)
    flyEnabled = on
...
makePill(124, "👻", "XUYEN TUONG", Color3.fromRGB(180, 60, 220), function(on)
    noclipEnabled = on
    if on then startNoclipInternal() else stopNoclipInternal() end
end)

section(187, "TOC DO BAY")
makeSlider(203)

section(288, "LUU VI TRI & TELEPORT")
...
hint.Text                 = "WASD | Space = len | Ctrl = xuong | Shift = boost x2.5"
...
print("[FlyTool Pro] Loaded! Tu dong bat lai sau khi chet.")
